...
#pragma mark 私钥签名 签名参数未知什么意思  （暂不可用）
...
unsigned char *rsaPKCS1SignData(const unsigned char *src,int src_len,unsigned char *pri_key,int pri_key_len, int *dst_len){
    ...
    if((bio_private = BIO_new_mem_buf(pri_key, -1)) == NULL)
    {
...
char *js_privatestring_decrypt(const char *cipher_text, char *private_key_string) {
    ...
    BIO *bio = NULL;
    RSA *rsa_privateKey = NULL;
    if((bio = BIO_new_mem_buf(private_key_string, -1)) == NULL)
    {
        printf("BIO_new_mem_buf failed!");
        BIO_free_all(bio);
...
char *js_publicstring_encrypt(const char *plain_text, char *public_key_string) {
    ...
    BIO *bio = NULL;
    RSA *rsa_publicKey = NULL;
    if((bio = BIO_new_mem_buf(public_key_string, -1)) == NULL)
    {
        printf("BIO_new_mem_buf failed!");
        BIO_free_all(bio);
